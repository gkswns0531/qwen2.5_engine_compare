    ...
    def generate(self, 
        ...
        for i, prompt in enumerate(prompts):
            ...
            if self.tokenizer:
                ...
                # Rough estimation (English: 4 chars=1 token, Korean: 2 chars=1 token)
                input_tokens = len(formatted_prompt.encode('utf-8')) // 3
            
            # Start generation
            start_time = time.time()
            
            # Accurate TTFT measurement: generate only first token
            ...
            # Process results
            if full_outputs and len(full_outputs) > 0:
                ...
                if hasattr(output, 'outputs') and len(output.outputs) > 0:
                    ...
                    if hasattr(output.outputs[0], 'token_ids'):
                        ...
                        if self.tokenizer:
                            ...
                            output_tokens = len(generated_text.encode('utf-8')) // 3
                else:
                    generated_text = ""
                    output_tokens = 0
            else:
            ...
            # Calculate TPS (total output tokens / total time)
...
def main():
    ...
    parser.add_argument("--engine_dir", type=str, 
                       default="./qwen2.5-1.5b-instruct_engine_fp8",
                       help="TensorRT-LLM engine directory path (default: FP8 Ultra engine)")
