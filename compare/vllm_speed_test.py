...
def run_streaming_test(
    ...
    # Add optional parameters (only for real quantization methods)
    if quantization and quantization not in ["fp16", "bf16", "float16", "bfloat16"]:
    ...
    for i, (prompt, formatted_prompt) in enumerate(zip(prompts, formatted_prompts)):
        ...
        # FP8 verification for logging only (first prompt)
        if i == 0 and quantization == "fp8" and not fp8_verification_done:
            verification_outputs, verification_info = verify_fp8_inference(llm, formatted_prompt, sampling_params)
            fp8_verification_done = True
            print(f"   FP8 verification completed: {verification_info['inference_time']:.3f}s")
...
def main():
    ...
    parser.add_argument("--quantization", type=str, default=None,
                       help="Quantization method (e.g., fp16, fp8, awq, None for auto)")
