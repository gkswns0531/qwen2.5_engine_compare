...
# TensorRT-LLM PyTorch Backend imports (official way)
import tensorrt_llm
...
def run_pytorch_backend_test(
    ...
    quantization: str = "fp8",  # Add FP8 quantization support
    ...
    for i, (prompt, formatted_prompt) in enumerate(zip(prompts, formatted_prompts)):
        ...
        # Token count calculation (accurate method)
        # Try to get exact token counts
        if hasattr(output, 'prompt_token_ids'):
            input_tokens = len(output.prompt_token_ids)
        else:
            input_tokens = len(llm.tokenizer.encode(formatted_prompt))
        
        if hasattr(output.outputs[0], 'token_ids'):
            output_tokens = len(output.outputs[0].token_ids)
        else:
            output_tokens = len(llm.tokenizer.encode(generated_text))
        
        # Memory info after
        mem_after_alloc, mem_after_reserved = get_memory_info()
        
        # Total time and TPS calculation
...
def main():
    ...
    parser.add_argument("--kv-cache-fraction", type=float, default=0.6,
                       help="KV cache GPU memory fraction (default: 0.6)")
    ...
    parser.add_argument("--quantization", type=str, default="fp8",
                       help="Quantization method (fp8, fp16, bf16)")
